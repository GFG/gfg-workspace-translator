...
class Workspace(Service):
    ...
    def listen(self, space_id, event_types, topic_id):
        try:
            response = self.client.subscriptions().create(body={
            ...
            try:
                ...
                response = self.renew(current_subscription)
            except Exception:
                raise e
            try:
                subscription_name = response['response']['name']
                assert response['done'], "Renewal not done"
                print(f'Renewed subscription: {subscription_name}')
                return subscription_name
            except Exception:
                print("Renew response:", response)
                raise e
